...
@synthesize fromEmail, toEmail, relayHost, SSLSwitch, useAuthSwitch, login, password, subject, messageBody, sig, sendImageSwitch, sendVCFSwitch, Spinner, ProgressBar, HighestState, prefKeyDictionary;
...
- (void)dealloc
{
    ...
    [relayHost release];
    [SSLSwitch release];
    [useAuthSwitch release];
    [login release];
    [password release];
    [subject release];
    [messageBody release];
    [sig release];
    [sendImageSwitch release];
    [sendVCFSwitch release];
    [Spinner release];
    [ProgressBar release];
    [prefKeyDictionary release];
    [super dealloc];
}
...
- (IBAction)sendMail:(id)sender 
{
    ...
    test_smtp_message.fromEmail = self.fromEmail.text;
    test_smtp_message.toEmail = self.toEmail.text;
    test_smtp_message.relayHost = self.relayHost.text;
    test_smtp_message.requiresAuth = self.useAuthSwitch.on;
    test_smtp_message.login = self.login.text;
    test_smtp_message.pass = self.password.text;
    test_smtp_message.wantsSecure = self.SSLSwitch.on; // smtp.gmail.com doesn't work without TLS!
    test_smtp_message.subject = self.subject.text;
