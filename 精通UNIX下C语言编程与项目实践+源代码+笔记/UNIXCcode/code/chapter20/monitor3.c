...
void timefunc(int sig)						/* 基本定时器响应函数 */
{
	...
	for (i=0; i<nTopTimer; i++)						/* 遍历模拟定时器数组 */
	{
		...
		{
			WriteMsg(msgid, sTimer+i, sizeof(SIMTIMER), i+1);			
...
{
	int msgkey;
	ASSERT(GetConfigValue("./config.ini", "LOCAL", "MSG", &msgkey, STRINT) == 0);	/* 读取消息队列关键字 */
	ASSERT((msgid = msgget(msgkey, 0666|IPC_CREAT)) > 0);
	return 0;	
}
...
int main()
{
	...
	ASSERT(GetMsg() == 0);				/* 打开消息队列 */
	ASSERT(InitServer() == 0);			/* 主机进程转后台执行 */
	signal(SIGALRM, timefunc);
	ASSERT(signal(SIGCLD, SIG_IGN) != SIG_ERR);	/* 预防僵死进程 */
	alarm(1);										/* 基本定时器首次定时 */
	while(1)								/* 主进程保持运行、不退出的状态 */
	{
		memset(&sTimer1, 0, sizeof(sTimer1));
		nSize = sizeof(sTimer1);
		nType = 0;
		ASSERT(ReadMsg(msgid, &sTimer1, &nSize, &nType) == 0);
		FuncAlarm(sTimer1);
	}
}
