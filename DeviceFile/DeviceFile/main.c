...
////该文件就是定义 termios 结构体的文件，，对该结构的实现有详细的解释
//#ifndef _TERMIOS_H
//#define _TERMIOS_H
...
//// 在设置终端 termios 的信息之前，需要先等待输出队列中所有数据处理完(耗尽)。对于修改参数会影响输出的情况，就需要使用这种形式(参见 tcsetattr()，TCSADRAIN 选项)。
...
//// 在设置终端 termio 的信息之前，需要先等待输出队列中所有数据处理完(耗尽)。对于修改参数会影响输出的情况，就需要使用这种形式(参见 tcsetattr()，TCSADRAIN 选项)。
//#define TCSETAW 0x5407
//// 在设置 termio 的信息之前，需要先等待输出队列中所有数据处理完，并且刷新(清空)输入队列。再设置（参见 tcsetattr()，TCSAFLUSH 选项）。
//#define TCSETAF 0x5408
//// 等待输出队列处理完毕(空)，如果参数值是0，则发送一个 break（参见 tcsendbreak()，tcdrain()）。
//#define TCSBRK 0x5409
//// 开始/停止控制。如果参数值是0，则挂起输出；如果是1，则重新开启挂起的输出；如果是2，则挂起输入；如果是3，则重新开启挂起的输入（参见 tcflow()）。
//#define TCXONC 0x540A
////刷新已写输出但还没发送或已收但还没有读数据。如果参数是0，则刷新(清空)输入队列；如果是1，则刷新输出队列；如果是2，则刷新输入和输出队列（参见 tcflush()）。
...
////#define TIOCGPGRP 0x540F// 设置指定终端设备进程的组 id(参见 tcsetpgrp())。
...
////#define VKILL 3 // c_cc[VKILL] = KILL (^U)，\025，终止字符。#define VEOF 4 // c_cc[VEOF] = EOF (^D)，\004，文件结束字符。
...
////#define BRKINT 0000002 // 在 BREAK 时产生 SIGINT 信号。#define IGNPAR 0000004 // 忽略奇偶校验出错的字符。
...
////#define ONLCR 0000004 // 在输出时将换行符 NL 映射成回车-换行符 CR-NL。#define OCRNL 0000010 // 在输出时将回车符 CR 映射成换行符 NL。
////#define ONOCR 0000020 // 在0 列不输出回车符 CR。
...
////#define TAB3 0014000 // 水平制表延迟类型3。#define XTABS 0014000 // 将制表符 TAB 换成空格，该值表示空格数。
...
////#define B110 0000003 // 波特率 110。#define B134 0000004 // 波特率 134。
...
////#define CS7 0000040 // 每字符7 比特位。#define CS8 0000060 // 每字符8 比特位。
...
//#define XCASE 0000004 // 若设置了 ICANON，则终端是大写字符的。#define ECHO 0000010 // 回显输入字符。
////#define ECHOE 0000020 // 若设置了 ICANON，则 ERASE/WERASE 将擦除前一字符单词。
...
////#define ECHOCTL 0001000 // 若设置了 ECHO，则除 TAB、NL、START 和 STOP 以外的 ASCII控制信号将被回显成象^X 式样，X 值是控制符+0x40。
...
////#define PENDIN 0040000 // 当下一个字符是读时，输入队列中的所有字符将被重显。#define IEXTEN 0100000 // 开启实现时定义的输入处理。
...
////#define TCOON 1 // 重启被挂起的输出。#define TCIOFF 2 // 系统传输一个 STOP 字符，使设备停止向系统传输数据。
...
////extern speed_t cfgetispeed (struct termios *termios_p);// 返回 termios_p 所指 termios 结构中的发送波特率。
